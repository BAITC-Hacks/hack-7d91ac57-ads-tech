...
`HackAlem AI 2026` · `Трек 05 · Логистика` · `Кейс ТОО «Электрокомплект» (ekt.kz)` · `Команда Ads Tech` · Статус: **работает, проверено 23.09.2026 в 17:00 (Docker с нуля, 11 тестов, verify.sh)**
...
|---|---|---|
| 0 | Открыть http://localhost:8080, войти `manager` / `demo` | Экран входа в стиле ЭК; после входа обучение из 15 шагов (можно пропустить) |
...
3. `cd backend && .venv/bin/python -m pytest tests -q` — 11 тестов зелёные (тест импорта партнёра пропускается, если файлов партнёра нет).
