...
`HackAlem AI 2026` · `Трек 05 · Логистика` · `Кейс ТОО «Электрокомплект» (ekt.kz)` · `Команда Ads Tech` · Статус: **работает, проверено <время>**

| Демо | Видео (60 сек) | Скриншот |
|---|---|---|
| локально, 3 команды ниже | <ссылка> | ![Главный экран](docs/screenshot.png) |
...
**Что вы увидите:** дашборд с 41 рекомендованной позицией по 6 поставщикам на синтетических данных «Электрокомплекта» (72 артикула, 24 месяца продаж); у каждой строки количество, срочность и обоснование словами; по клику график продаж с выбросами и дефицитом; кнопки «Утвердить» и «Экспорт для 1С».
...
| | Было | Стало |
|---|---|---|
| Время расчёта по складу | 3–4 часа в Excel, раз в месяц | 2 секунды, хоть каждое утро |
...
| Сезонность освещения (ноябрь ×1.35, июнь ×0.67) | усреднена | учтена по месяцам горизонта поставки |
...
- [x] 6 автотестов, повторяющих 5 проверок «must have» из ТЗ плюс запрет автоотправки
...
- [ ] Дашборд менеджера (React): таблица по поставщикам, график по артикулу, утверждение, экспорт, панель ассистента — <статус к 17:40>
...
| 2 | Склад «Главный», нажать «Рассчитать» | Сводка: 41 позиция, 6 поставщиков, 17 критичных; таблица сгруппирована по поставщикам |
| 3 | Раскрыть строку `LMP-012 Светильник NLED 24W` | Обоснование: спрос/день, сезонный коэффициент сентября, план +12%/год, прогноз на 59 дн., страховой запас, остаток, потребность, «критично» |
...
| 6 | Спросить ассистента «почему по LMP-012 столько?» | Шаг `explain_sku` в панели, ответ с числами, `latency_ms` |
| 7 | Изменить количество, нажать «Утвердить заказ» поставщику | Подтверждение, заказ появляется в «Утверждённых», надпись «не отправляется автоматически» |
| 8 | «Экспорт XLSX» | Файл `orders-….xlsx` с колонками Поставщик, Артикул, Наименование, Количество, Срочность, Обоснование |

**Тестовые данные:** `backend/data/sample/` — синтетика с реалистичной структурой (разрешено ТЗ), генератор `backend/app/synth.py`, seed 42. Клиенты обезличены (`C001…C200`). Файл `_truth_oneoffs.csv` содержит «правду» о вставленных разовых заказах для проверки.
...
curl -s -X POST localhost:8000/api/replenish/whatif -H 'Content-Type: application/json' -d '{"sku":"LMP-012","in_transit":300}' | python3 -c "import sys,json; print(json.load(sys.stdin)['delta_qty'])"
...
1. **Выявление разовых крупных заказов.** По транзакциям считается медиана и MAD дневных продаж. Транзакция помечается разовой, если робастный z-score `0.6745·(qty − медиана)/MAD > 5` и `qty > 3·медиана` (правило A), либо если один клиент дал ≥ 60 % продаж месяца, месяц > 2× медианного, а его покупка > 3× типичной (правило B, «крупная продажа одному клиенту»). Помеченные транзакции в регулярном спросе заменяются типичным дневным объёмом, сами заказы сохраняются и показываются отдельно.
...
5. **Страховой запас.** `z(уровень сервиса) · σ(дневной спрос за 90 дн.) · √H`. Уровень сервиса по категории (Автоматика 98 %, Кабель/Освещение/Розетки 95 %, Щиты/Инструмент 90 %) или заданный вручную.
...
8. **Обоснование.** Шаблонный текст с фактическими числами по шагам 1–7 для каждой строки (не требует LLM).
...
| `DATA_DIR` | нет | `data/sample` | папка с CSV (см. форматы в разделе 3 и `docs/API.md`) |
...
1. `bash scripts/verify.sh` — health, расчёт, объяснение позиции, экспорт. Последняя строка `OK`.
2. Пройти [основной сценарий](#3-основной-сценарий-что-проверять) в UI.
3. `cd backend && .venv/bin/python -m pytest tests -q` — 6 тестов must-have зелёные.
...
- Прогноз: сезонные индексы + линейный тренд. Для длинной истории можно заменить на Holt-Winters/Prophet, интерфейс движка не изменится.
- Товары в пути учитываются, если ETA попадает в горизонт; частичные поставки не моделируются.
- Утверждённые заказы хранятся в JSON-файле, не в учётной системе.

**Дальнейшее развитие:**
- Интеграция с 1С: импорт материальной ведомости по расписанию, выгрузка утверждённого заказа документом «Заказ поставщику».
- Приоритизация по стоимости дефицита (маржа × упущенные продажи), учёт условий поставщика (скидки от объёма, кратность паллет).
...
|---|---|---|
| Дархан Нурикенов | архитектура, движок расчёта, API, тесты, README | [@Nurikenov](https://github.com/Nurikenov) |
| <имя> | <роль> | |
| <имя> | <роль> | |
