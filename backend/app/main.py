...
SYSTEM_PROMPT = (
    ...
    "Если «что если» (в пути, остаток) — what_if, и передавай в него только то, что пользователь назвал сам; «придёт ещё N» — add_in_transit. "
    "Письмо поставщику — draft_supplier_email (это только черновик): покажи текст черновика как есть, без пересказа."
)
