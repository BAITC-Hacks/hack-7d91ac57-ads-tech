...
RU_GEN = {"января": 1, "февраля": 2, "марта": 3, "апреля": 4, "мая": 5, "июня": 6, "июля": 7, "августа": 8, "сентября": 9, "октября": 10, "ноября": 11, "декабря": 12}


def iek_lead_time(path: Path) -> tuple[int | None, list[dict]]:
    """Lead time from «Путь ИЭК» headers: «… от 7 сентября 2026 г. (поступление до 01.10.2026)».
    Weighted median of (ETA − document date) over transit lines."""
    d = pd.read_excel(path)
    samples: list[tuple[int, int]] = []
    docs = []
    for c in d.columns[3:]:
        h = str(c).replace("\xa0", " ")
        m1 = re.search(r"от (\d{1,2}) ([а-я]+) (\d{4})", h)
        m2 = re.search(r"поступление до (\d{2})\.(\d{2})\.(\d{4})", h)
        if not (m1 and m2) or m1.group(2) not in RU_GEN:
            continue
        doc = date(int(m1.group(3)), RU_GEN[m1.group(2)], int(m1.group(1)))
        eta = date(int(m2.group(3)), int(m2.group(2)), int(m2.group(1)))
        lines = int((pd.to_numeric(d[c], errors="coerce").fillna(0) > 0).sum())
        days = (eta - doc).days
        docs.append({"document": h.split(" (")[0].strip(), "date": str(doc), "eta": str(eta), "days": days, "lines": lines})
        if lines and days > 0:
            samples.append((days, lines))
    if not samples:
        return None, docs
    samples.sort()
    total = sum(n for _, n in samples)
    acc = 0
    for days, n in samples:
        acc += n
        if acc * 2 >= total:
            return days, docs
    return samples[-1][0], docs
...
CATEGORY_RULES = [
    ("Аксессуары к автоматам", ["расцепитель", "доп.", "доп ", "катушка", "электропривод", "аварийно-доп", "контакт доп", "блок-контакт"]),
    ("Автоматы", ["armat", "ва47", "ва 47", "ва88", "ва 88", "автомат", "выключатель автомат", "ва-", "ba 47", "ba47"]),
    ...
    ("Кнопки и сигнальная арматура", ["кнопка", "кнопк", "микр", "сигнальн", "лампа сигн", "пост кноп", "переключатель кулачк", "ад-22", "ас-22"]),
    ("Предохранители и разъединители", ["предохранитель", "разъединитель", "рубильник", "вставка плавк", "ппн"]),
    ("Преобразователи частоты и УПП", ["преобраз", "упп", "устройство плавного"]),
    ("Элементы питания", ["батарейка", "аккумулятор"]),
    ("Реле и автоматика", ["датчик", "стаб", "реле", "таймер", "фотореле", "термостат", "гигростат", "логическое", "ограничитель", "опс", "уздп", "стабилизатор"]),
    ...
    ("Щиты и корпуса", ["щит", "щрн", "щрв", "щмп", "щу-", "щу ", "бокс", "корпус", "кмпн", "шкаф", "ящик", "щурн", "щурв", "щэ", "сф-0", "замок", "основание"]),
    ("Кабель и провод", ["кабель ", "провод", "utp", "ftp", "ввг", "пвс", "шввп", "сип", "кг ", "nym", "пугв", "пв-", "шнур"]),
    ("Кабеленесущие системы", ["кабель-канал", "лоток", "труб", "сальник", "гофр", "короб", "профиль", "strut", "консоль", "стойка", "кронштейн", "крышка", "угол", "поворот", "заглушка", "перегородка", "т-", "соединитель", "разветвитель"]),
    ("Клеммы и наконечники", ["у03", "у04", "у05", "изолятор", "клемм", "наконечник", "гильза", "зажим", "зви", "зни", "зои", "шина", "колодка", "перемычка", "сизы", "разъем", "разъём", "вилка", "коробка", "установочная"]),
...
def build(iek_dir: Path, se_dir: Path, out: Path) -> dict[str, int]:
    ...
    products = pd.DataFrame({
        ...
        "category": [category_from_name(names.get(s, "")) for s in skus],
        "class_2026": [se_cat.get(s, "") if sup_of.get(s) == "SE" else "" for s in skus],
    ...
    lt, lt_docs = iek_lead_time(_find(iek_dir, "путь"))
    if lt:
        suppliers.loc[suppliers["supplier_id"] == "IEK", "lead_time_days"] = lt
    notes = [
        f"Срок поставки IEK = {lt} дн.: взвешенная медиана «дата документа → поступление» по {sum(x['lines'] for x in lt_docs)} строкам «Путь ИЭК» ({', '.join(str(x['days']) for x in lt_docs)} дн. по документам)." if lt else "Срок поставки IEK = 30 дн. (допущение: даты в «Путь ИЭК» не распознаны).",
        "Срок поставки Systeme Electric = 30 дн. — допущение, в выгрузке дат заказа нет; редактируется в suppliers.csv.",
        "Остаток IEK на 23.09 = остаток на 01.09 минус отгрузки сентября; поступления сентября в выгрузке отсутствуют, поэтому остаток может быть занижен.",
        "Остаток Systeme Electric = «Свободный остаток» на 22.09.2026 (с учётом резервов).",
        "Клиент в выгрузке отсутствует: «один клиент» = одна расходная накладная.",
        "Себестоимость есть только у Systeme Electric («СС реал»); у IEK суммы в тенге не считаются.",
        "Продажи 2024 года дозаполнены из помесячной таблицы равномерно по дням; транзакции есть с 2025 года.",
    ]
    import json as _json
    (out / "meta.json").write_text(_json.dumps({"source": "Выгрузки 1С ТОО «Электрокомплект»: IEK и Systeme Electric", "as_of": str(TODAY), "notes": notes, "iek_lead_time_docs": lt_docs}, ensure_ascii=False, indent=1))
